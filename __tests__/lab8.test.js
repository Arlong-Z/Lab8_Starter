...
describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const text_value = await text.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cart_count = await (await page.$("#cart-count")).getProperty("innerText");
    const counts = await cart_count.jsonValue();
    expect(counts).toBe("20");
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < items.length; i++){
      ...
      // Check to see if the button say "Remove from Cart"
      expect(text_value).toBe("Remove from Cart");
    }
    // Check to see if the innerText of #cart-count is 20
    const cart_count = await (await page.$("#cart-count")).getProperty("innerText");
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < items.length; i++){
      ...
      // Check to see if the button say "Add to Cart"
      expect(text_value).toBe("Add to Cart");
    }
    // Check to see if the innerText of #cart-count is 0
    const cart_count = await (await page.$("#cart-count")).getProperty("innerText");
    const counts = await cart_count.jsonValue();
